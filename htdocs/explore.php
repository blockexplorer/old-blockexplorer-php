...
if(!$db)
{
	error("Database connection timeout (likely due to overload). Try again later.",503);
	goto headerend;
}
...
if($page=="search"||$page=="searchgo")
{
	...
	if(count($transactions)+count($blocks)+count($addresses)==0)
	{
		if(strlen($input)==130&&preg_match("/[0-9a-f]{4,130}/",$input))
		{
			...
			redirect("/address/$input");
			die();
		}
		if(strlen($input)==40&&preg_match("/[0-9a-f]{4,130}/",$input))
		{
			$input=hash160ToAddress($input);
			redirect("/address/$input");
			die();
		}
		if(checkAddress($originput))
		{
			redirect("/address/$originput");
			die();
		}
	}
}
...
if($page=="t")
{
	if(preg_match("/^[1-9A-HJ-NP-Za-km-z]{7,20}$/",$param1)==1)
	{
		...
		if($num_rows>1)
		{
			...
			{
				$thistx=$onetx["hash"];
				$error.="<li><a href=\"/tx/$thistx\">$thistx</a></li>\n";
				$onetx=pg_fetch_assoc($hash);
			}
			$error.="</ul>\n";
		}
	...
	if(preg_match("/^[1-9A-HJ-NP-Za-km-z]{7,20}$/",$param1)==1)
	{
		...
		if($num_rows>1)
		{
			...
			while($oneaddr)
			{
				...
				$error.="<li><a href=\"/address/$thisaddr\">$thisaddr</a></li>\n";
				$oneaddr=pg_fetch_assoc($address);
			}
			$error.="</ul>\n";
		}
...
if($page=="block")
{
	$cache=cacheget($page.md5($param1));
	if($cache!==false)
	{
		echo $cache;
		goto end;
...
if($page=="address")
{
	...
	//get address
	$address=$param1;
	if(!preg_match('/^[1-9A-HJ-NP-Za-km-z]+$/',$address)==1 ||strlen($address)>36||!checkAddress($address))
	{
		error("Invalid address",400);
		goto headerend;
	}
	$insertrss="{$scheme}blockexplorer.com/rssa/$address.xml";
	...
	if($firstseen!==false&&!is_null($firstseen))
	{
		...
		$blockstring="<a href=\"/block/$firstseen\">Block $blocknum</a> ($blocktime)";
	}
	else
	{
...
if($page=="rssa")
{
	...
	$address=substr($param1,0,strlen($param1)-4);
	if(!preg_match('/^[1-9A-HJ-NP-Za-km-z]+$/',$address)==1 ||strlen($address)>36||!checkAddress($address))
	{
		senderror(400);
		header("Content-type: text/plain");
	...
	$query=pg_query_params($db,"SELECT to_char(blocks.time AT TIME ZONE 'UTC','Dy, DD Mon YYYY HH24:MI:SS +0000') AS time,outputs.value AS value,outputs.index AS oid,blocks.number AS number,encode(outputs.tx,'hex') AS tx FROM outputs JOIN blocks ON outputs.block=blocks.hash WHERE outputs.hash160=decode($1,'hex') ORDER BY outputs.id DESC LIMIT 20;",array($hash160));
	...
	echo "<link>{$scheme}blockexplorer.com/address/$address</link>"."\n";
	echo "<title>BBE - $address</title>"."\n";
	echo '<atom:link href="'."{$scheme}blockexplorer.com/rssa/$address.xml".'" rel="self" type="application/xml" />'."\n";
	if(isset($builddate))
	{
	...
	while($onetx)
	{
		...
		echo "<guid>{$scheme}blockexplorer.com/address/$address#$anchor</guid>\n";
		echo "<pubDate>$time</pubDate>\n";
		echo "<title>Received $amount BTC</title>\n";
		echo "<link>{$scheme}blockexplorer.com/address/$address#$anchor</link>"."\n";
		echo "</item>\n";
		$onetx=pg_fetch_array($query);
	}
	//end content
...
if($page=="robots.txt"&&(!isset($senthost)||$senthost!=$my_name))
{
...
?>
<title><?php echo "$title - Bitcoin Block Explorer"; ?></title>
<style type="text/css">
...
if($page=="search")
{
	...
	if(!empty($addresses))
	{
		...
		{
			echo "<li><a href=\"/address/$i\">$i</a></li>\n";
			$donesomething=true;
		}
		echo "</ul>\n";
	}
...
echo '
	...
	<p>Some data from Bitcoin Block Explorer is available through the machine-readable <a href="/q">Real-Time Stats pages</a>.
	...
	echo '<p>"Go" takes you to the first result found even when your search could be ambiguous, and it doesn\'t search for public keys.</p>'."\n";
	echo '<form action="/search" method="post" ><p><input type="text" name="q" size="50"> <input type="submit" name="searchgo" value="Go"> <input type="submit" value="Search"></p></form>'."\n";
	...
	while($oneblock)
	{
		...
		echo "<tr>\n";
		echo "<td><a href=\"/block/$hash\">$number</a></td>\n";
		echo "<td><a href=\"/block/$hash\">$hashtrunc</a></td>\n";
		echo "<td>$time</td>\n";
		echo "<td>$total</td>\n";
		echo "<td>$totalbtc</td>\n";
		echo "<td>$size</td>\n";
	...
	{
		$txhash=$onetx["hash"];
		$txhashtrunc="<a href=\"/tx/$txhash\">".substr($txhash,0,10)."...</a>";
		$amount=removeTrailingZeroes($onetx["totalvalue"]);
		$blockhash=$onetx["blockhash"];
		$blockview="<a href=\"/block/$blockhash\">".$onetx["blocknum"]."</a>";
		$time=$onetx["time"];
	...
	echo '<p>Largest transactions and strange transactions have been temporarily moved <a href="/txstats">here</a>.</p>';
...
if($page=="block")
{
	...
	{
		echo "<li>Previous block".help("Every block builds on another, forming a chain. This is the full hash of the previous block.").": <a href=\"/block/$prev\">$prev</a></li>\n";
	}
	if($next!=false&&!is_null($next))
	{
		echo "<li>Next block".help("The full hash of the block that will build onto this one. This field is not included in real blocks.").": <a href=\"/block/$next\">$next</a></li>\n";
	}
	...
	echo "<li><a href=\"/rawblock/$block\">Raw block</a>".help("Almost the same as getblock's output.")."</li>\n";
	echo "</ul>";
	...
	{
		$hash=$row["hash"];
		$hashtrunc="<a href=\"/tx/$hash\">".substr($hash,0,10)."...</a>";
	...
	if(!isset($donotcache))
	{
		cacheput("block".md5($param1),ob_get_flush(),300,VERSION);
	}
	else
	{
		cacheput("block".md5($param1),ob_get_flush(),300,VERSION."o",0);
	}
}
...
if($page=="tx")
{
	...
	echo "<li>Appeared in <a href=\"/block/{$tx["block"]}\">block {$tx["blocknumber"]}</a> ({$tx["time"]})</li>\n";
	...
	echo "<li><a href=\"/rawtx/$hash\">Raw transaction</a>".help("Almost the same as getblock.")."</li>\n";
	...
	if($onedup)
	{
		...
		while($onedup)
		{
			...
			$duplicate.="<a href=\"/block/$blockhash\">".substr(removeLeadingZeroes($blockhash),0,10)."</a>";
			$onedup=pg_fetch_assoc($duplicates);
		}
		$duplicate.="</li>";
	}
	...
	while($oneinput)
	{
		...
		if(!is_null($prev))
		{
		echo "<td><a name=\"i$myid\" href=\"/tx/$prev#o$previndex\">$prevtrunc</a></td>\n";
		}
		else
		{
	...
	while($oneoutput)
	{
		...
		if($redeemed!==false)
		{
			$rtx=$redeemed["tx"];
			$rid=$redeemed["id"];
			$rtxtrunc="<a name=\"o$index\" href=\"/tx/$rtx#i$rid\">".substr($rtx,0,12)."...</a>";
		}
		else
		{
	...
	cacheput("tx".md5($param1),ob_get_flush(),300,VERSION);
}
...
if($page=="address")
{
	...
	while($txcounter<=$txlimit)
	{
		...
		if($cord=="Received")
		{
			...
			$txtrunc="<a name=\"$anchorname\" href=\"/tx/$tx#o$index\">".substr($tx,0,10)."...</a>";
		}
		else
		{
			$anchorname=substr($tx,0,16)."i$index";
			$txtrunc="<a name=\"$anchorname\" href=\"/tx/$tx#i$index\">".substr($tx,0,10)."...</a>";
		}
		...
		$blockstring="<a href=\"/block/$block\">Block $blocknum</a> ($txtime)";
		...
		if($cord=="Received")
		{
			...
			while($thisio)
			{
				...
				$addressstring="<a href=\"/address/$address\">$address</a>";
